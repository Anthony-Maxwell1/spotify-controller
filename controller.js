  ...
  ws.onmessage = async (msg) => {
    ...
    switch (cmd.action) {
      ...
      case "previous":
        ...
        break;
      case "listPlaylists":
        sendResponse({ action: "listPlaylists", data: await getPlaylists() });
        break;
      case "getPlaylist":
        sendResponse({
          action: "getPlaylist",
          data: await getPlaylist(cmd.uri),
        });
        break;
      case "getLikedSongs":
        sendResponse({ action: "getLikedSongs", data: await getLikedSongs() });
        break;
      case "getSong":
        sendResponse({ action: "getSong", data: getSong(cmd.uri) });
        break;
      case "playFromUri":
        Spicetify.Player.playUri(cmd.uri);
        break;
      case "fetchWithAuth":
        sendResponse({
          action: "fetchWithAuth",
          data: await fetchWithAuth(cmd.url, cmd.method, cmd.headers, cmd.body),
        });
        break;
      case "getStatus":
        sendResponse({ action: "getStatus", data: await getStatus() });
        break;
      case "setShuffle":
        setShuffle(cmd.shuffle);
        break;
      case "setRepeat":
        setRepeat(cmd.repeat);
        break;
      case "setVolume":
      ...
      case "like":
        ...
        break;
      case "seek":
        seek(cmd.position ?? cmd.milliseconds ?? 0);
        break;
      case "searchSpotify":
        sendResponse({
          action: "searchSpotify",
          data: await gqlSearch(cmd.query, cmd.limit),
        });
        break;
    }
  };
...
async function getPlaylists() {
  ...
  return [
    ...
    ...playlistsResponse.items.map((playlist) => ({
      ...
      coverArtUrl: playlist.images[0]
        ? "https://mosaic.scdn.co/640/" +
          playlist.images[0]?.url.split(":").slice(2).join("")
        : "",
    })),
  ];
}
...
async function getLikedSongs() {
  if (!Spicetify) return null;
  const likedSongs = await Spicetify.CosmosAsync.get(
    "sp://core-collection/unstable/@/list/tracks/all",
    { policy: { link: true } },
  );
...
async function gqlSearch(query, limit = 30) {
  ...
  function formatResults({ tracks, albums, artists }) {
    const t = tracks.map((t) => ({
      ...
      subtitle: t.item.data?.artists?.items
...
async function getStatus() {
    ...
    state.nextItems?.map((track) => ({
      ...
      albumArtUrl: track.album?.images?.[0]?.url
        ? "https://i.scdn.co/image/" +
          track.album.images[0].url.split(":").pop()
        : "",
  ...
  if (playlistUri.startsWith("spotify:playlist:")) {
    try {
      const playlist =
        await Spicetify.Platform.PlaylistAPI.getPlaylist(playlistUri);
      playlistName = playlist?.name ?? "";
    } catch {}
  }

  const isSongLiked = current.uri ? Spicetify.Player.getHeart() : false;
